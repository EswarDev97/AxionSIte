		...
		$.extend(Plugin.prototype, {
				...
				bindEvents: function(el) {
					...
					.closest('.'+this._name+'-container').find('input').each(function(){
						...
						$(this).on('keydown', function(e) {
							...
							if (inputId != 'payfieldCardExpirationMonth')
							{
								...
								let month = $(this).val()+String.fromCharCode(e.which);

								if (month.length > 1)
								{
									// If not tab, delete and backspace key and not an integer, prevent keypress
									if (e.which !== 39 && e.which !== 37 && e.which !== 9 && e.which !== 8 && e.which !== 46 && !pattern.test(month)) {										
										e.preventDefault();
										$(this).val('');
						...
						$(this).on('blur', function(e) {
							...
							if (inputId == 'payfieldCardExpirationMonth')
							{
								if (inputValue != undefined && inputValue != '')
								{
									...
									{
										inputValue = '0' + $(this).val();
										$(this).val(inputValue.slice(-2));
									}
								}
							}
						});
				...
				wrapInputElement: function(el) {
					...
					el.addClass(this._name+'-input') // Add a class to the input with the plugin name
					  ...
					  .attr('pattern', '^[0-9\\s]*$') // Allow only integers in the card input
